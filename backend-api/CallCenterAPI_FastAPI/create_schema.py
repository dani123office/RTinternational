...
statements = [
    # === USERS (no self-referencing FK yet) ===
    """CREATE TABLE users (
        id                  INT             NOT NULL IDENTITY(1,1),
        name                VARCHAR(100)    NOT NULL,
        email               VARCHAR(150)    NOT NULL,
        password_hash       VARCHAR(255)    NOT NULL,
        role                VARCHAR(10)     NOT NULL DEFAULT 'agent' CHECK (role IN ('admin','manager','agent')),
        manager_id          INT             NULL,
...
END""",
    ...
    """CREATE TABLE customers (
        ...
        utility_type        VARCHAR(15)     NOT NULL DEFAULT 'electricity' CHECK (utility_type IN ('electricity','gas','both')),
        created_at          DATETIME2       NOT NULL DEFAULT GETDATE(),
        updated_at          DATETIME2       NOT NULL DEFAULT GETDATE(),
...
END""",
    ...
    """CREATE TABLE electricity_meters (
        ...
        meter_type          VARCHAR(15)     NOT NULL DEFAULT 'standard' CHECK (meter_type IN ('standard','day_night','half_hourly')),
...
END""",
    ...
    """CREATE TABLE call_backs (
        ...
        status                      VARCHAR(15)     NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','done','overdue','cancelled','not_interested')),
        outcome                     VARCHAR(20)     NULL CHECK (outcome IN ('interested','not_interested','no_answer','rescheduled','converted')),
...
END""",
    ...
    """CREATE TABLE transfers (
        ...
        utility_type                VARCHAR(15)     NOT NULL DEFAULT 'electricity' CHECK (utility_type IN ('electricity','gas','both')),
        status                      VARCHAR(20)     NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','in_progress','sale_complete','cancelled','approved','rejected','dispute','completed','failed','chasing','cotInProgress','hold','not_interested')),
...
END""",
    ...
    """CREATE TABLE sales (
        ...
        cot_status              VARCHAR(20)     NOT NULL DEFAULT 'submitted' CHECK (cot_status IN ('submitted','chasing','cot_in_progress','cot_complete','done','hold')),
        cot_date                DATE            NULL,
...
for i, stmt in enumerate(statements):
    ...
    try:
        ...
        errors += 1
        msg = str(e)
        print(f'Error {i}: {msg[:150]}')
        conn.rollback()
        break

if errors == 0:
    print('All tables created successfully!')
    cursor.execute("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='dbo' AND TABLE_TYPE='BASE TABLE' ORDER BY TABLE_NAME")
