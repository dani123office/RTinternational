...
const tabs = ['Overview', 'Transfers', 'Sales', 'Attendance', 'Campaigns']
...
export default function AgentDetail() {
  ...
  const loadCampaigns = useCallback(async () => {
    ...
    try {
      const res = await api.get(endpoints.campaigns.base, { params: { agentId: id } })
  ...
  const handleCampaignUpload = async (e) => {
    e.preventDefault()
    if (!campaignFile) return toast('Please select a CSV or TXT file', 'error')
    ...
    reader.onload = async (event) => {
      try {
        ...
        const isCSV = campaignFile.name.endsWith('.csv')
        let leads = []
        ...
        if (isCSV) {
          ...
          headers.forEach((h, idx) => {
            ...
            if (norm.includes('business') || norm.includes('company') || norm.includes('firm')) businessIdx = idx
            else if (norm.includes('owner') || norm.includes('contact') || norm.includes('name')) {
              if (ownerIdx === -1 && businessIdx !== idx) ownerIdx = idx
            }
            else if (norm.includes('phone') || norm.includes('mobile') || norm.includes('tel') || norm.includes('num')) phoneIdx = idx
            else if (norm.includes('postcode') || norm.includes('zip') || norm.includes('post') || norm.includes('addr')) postcodeIdx = idx
            else if (norm.includes('note') || norm.includes('comment') || norm.includes('info')) notesIdx = idx
          })
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            leads.push({
              businessName: row[businessIdx] || `Lead #${i}`,
              ownerName: ownerIdx !== -1 ? row[ownerIdx] || '' : '',
              phone: row[phoneIdx],
          ...
          // Text file - one phone per line
          lines.forEach((line, idx) => {
            const phone = line.trim()
            if (phone) {
              leads.push({
                businessName: `Lead #${idx + 1}`,
                ownerName: '',
                phone,
                postcode: '',
                notes: ''
        ...
        if (leads.length === 0) {
          toast('No valid phone numbers found in file', 'error')
          setUploadingCampaign(false)
          return
        }
        ...
        await api.post(endpoints.campaigns.base, payload)
        toast('Campaign uploaded and assigned successfully!', 'success')
        setCampaignFile(null)
        setCampaignName('')
        loadCampaigns()
      } catch (err) {
        toast(err.response?.data?.detail || 'Failed to upload campaign', 'error')
      } finally {
        setUploadingCampaign(false)
      }
    }
    reader.readAsText(campaignFile)
  }

  const handleDeleteCampaign = async (campaignId) => {
    if (!confirm('Are you sure you want to delete this campaign?')) return
    try {
      await api.delete(endpoints.campaigns.delete(campaignId))
              ...
              {activeTab === 'Campaigns' && (
                <div>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* Left Column: Upload New Campaign */}
                    <div className="lg:col-span-1">
                      <div className="rt-card" style={{ padding: '20px' }}>
                        <h4 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
                          <Plus size={16} className="text-indigo-600" /> Assign New Campaign
                        </h4>
                        <form onSubmit={handleCampaignUpload} className="flex flex-col gap-4">
                          <div>
                            <label className="rt-label">Campaign Name</label>
                            <input
                              type="text"
                              required
                              placeholder="e.g. London Plumbers - 10 July"
                              value={campaignName}
                          ...
                          <div>
                            <label className="rt-label">Upload CSV / TXT File</label>
                            <input
                              type="file"
                              required
                              accept=".csv,.txt"
                              onChange={(e) => {
                                ...
                                if (file && !campaignName) {
                                  // Auto-fill campaign name with file name without extension
                          ...
                          <p style={{ fontSize: '11px', color: '#64748b', margin: 0 }}>
                            Select an Excel CSV (Business Name, Owner, Phone, Postcode, Notes) or Notepad TXT list (one phone number per line).
                          </p>
                          <button
                            type="submit"
                            disabled={uploadingCampaign}
                            className="rt-btn-primary w-full"
                          >
                            {uploadingCampaign ? 'Uploading & Parsing...' : 'Assign to Agent'}
                          </button>
                        </form>
                      </div>
                    </div>

                    {/* Right Column: Campaign List & Progress Tracker */}
                    <div className="lg:col-span-2">
                      <div className="rt-card" style={{ padding: '20px', minHeight: '300px' }}>
                        <h4 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
                          <PhoneCall size={16} className="text-indigo-600" /> Assigned Campaigns Progress
                        </h4>
                        ...
                        {campaignsLoading ? (
                          ...
                          <div className="text-center py-12">
                            <p className="text-sm text-slate-400">No campaigns assigned to this agent yet.</p>
                          </div>
                        ) : (
                          <div className="flex flex-col gap-4">
                            {campaigns.map(c => {
                              const progress = c.totalLeads > 0 ? Math.round((c.calledLeads / c.totalLeads) * 100) : 0
                              return (
                                <div key={c.id} className="p-4 rounded-xl border border-slate-100 hover:border-slate-200 transition-all" style={{ background: '#f8fafc' }}>
                                  <div className="flex items-start justify-between gap-3 mb-3">
                                    <div>
                                      <h5 className="text-sm font-bold text-slate-800">{c.name}</h5>
                                      <span style={{ fontSize: '11px', color: '#94a3b8' }}>
                                        Assigned on {new Date(c.createdAt).toLocaleDateString('en-GB')}
                                      </span>
                                    </div>
                                    <button
                                      onClick={() => handleDeleteCampaign(c.id)}
                                      className="p-1.5 rounded-lg border-0 bg-red-50 text-red-500 cursor-pointer hover:bg-red-100 transition-colors"
                                  ...
                                  <div className="mb-3">
                                    <div className="flex items-center justify-between text-xs font-semibold text-slate-500 mb-1">
                                      ...
                                      <span>{c.calledLeads} / {c.totalLeads} Called</span>
                                    </div>
                                    <div style={{ width: '100%', height: '6px', background: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' }}>
                                      <div style={{ width: `${progress}%`, height: '100%', background: 'linear-gradient(135deg,#6366f1,#7c3aed)', borderRadius: '3px' }} />
                                    </div>
                                  </div>
