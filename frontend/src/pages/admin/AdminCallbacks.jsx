...
export default function AdminCallbacks() {
  ...
  const [fromDate, setFromDate] = useState(() => {
    ...
    if (dashboardMonth !== lastDashMonth) {
      sessionStorage.setItem('admin_callbacks_last_dashboard_month', dashboardMonth)
      if (dashboardMonth !== 'all') {
        const [year, month] = dashboardMonth.split('-').map(Number)
        const firstDay = `${year}-${String(month).padStart(2, '0')}-01`
        const lastDayDate = new Date(year, month, 0)
        const lastDay = `${year}-${String(month).padStart(2, '0')}-${String(lastDayDate.getDate()).padStart(2, '0')}`
        sessionStorage.setItem('admin_callbacks_from_date', firstDay)
        sessionStorage.setItem('admin_callbacks_to_date', lastDay)
        return firstDay
      } else {
        sessionStorage.setItem('admin_callbacks_from_date', '')
        sessionStorage.setItem('admin_callbacks_to_date', '')
        return ''
      }
    }
    return sessionStorage.getItem('admin_callbacks_from_date') || ''
  })
  const [toDate, setToDate] = useState(() => {
    return sessionStorage.getItem('admin_callbacks_to_date') || ''
  })
  ...
  const loadCallbacks = useCallback((p = 1) => {
    setLoading(true)
