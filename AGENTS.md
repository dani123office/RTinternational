...
40. **Blocked mobile check-in and check-out** — added user-agent based restriction on `/api/attendance/check-in` and `/api/attendance/check-out` endpoints. If request user-agent matches common mobile identifiers, backend returns a 400 Bad Request error. This applies to all roles (agents, managers, admins).
...
45. **Added Agent Totals Loan Summary & Breakdown View** — grouped multiple loan requests per agent into single aggregated agent summary rows in `PendingUsers.jsx`. Added view toggle between Agent Totals Summary and All Transactions, top loan KPI cards (Total Granted, Paid Back, Pending Balance), expandable inline breakdown drawers for individual loan details, and increased `/api/loans/all` per_page query limit to 500.
46. **Made all Dashboard Stat Cards Clickable** — updated `StatCard` in `Dashboard.jsx`, `components/manager/StatCard.jsx`, and `TeamDashboard.jsx` to accept `onClick` handlers and present interactive hover effects. Clicking "Today's Callbacks" or "Overdue" navigates to `/callbacks?filter=today` or `/callbacks?filter=overdue`, while "Transfers" and "Sales" navigate to their respective sections.
47. **Added Renewal Callbacks Feature** — created `renewalUtils.js` helper and `RenewalCallbacks.jsx` page view. Added a 6th stat card "Renewal Callbacks" to `Dashboard.jsx` and added "Renewal Callbacks" to the main sidebar menu for all roles. Automatically filters and highlights all customers whose energy contract end date (`contractEndDate`) has 30 days or less remaining, categorizing them into Urgent (≤15d), Upcoming (16-30d), and Expired, complete with direct action buttons to Schedule Callbacks or Submit Renewal Sales.
